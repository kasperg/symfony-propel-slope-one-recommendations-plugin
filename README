...
* Recommend other items with similar ratings to a specific item 

sfPropelSlopeOneRecommendation consists of Propel behaviors for retrieving recommendations and tasks
for maintaining the underlying datastructure for the recommendations.
...
* The MySQL task creates a *slope_one* stored procedure for maintaining the table and executes this. This will usually be faster than running the PHP task. This is recommended for production use if your MySQL version supports stored procedures (MySQL 5.1 and up).

		> php symfony slopeone:build-mysql
	
* The PHP task uses a combination of PHP and MySQL queries for maintaining the data.
...
* The MySQL approach:
		...
		[php]
		$builder = new SlopeOneMySBuilder(new SlopeOneSqlParser());
		$builder->build();
	
* The PHP approach:

		[php]
		$builder = new SlopeOnePhpBuilder(new SlopeOneSqlParser());
		$builder->build();
